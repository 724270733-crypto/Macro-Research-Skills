---
name: ifind-data
description: 基于同花顺 iFind API 的金融数据查询技能。用于获取 A 股、港股、美股的股票行情、基金净值、指数数据、财务报表等。触发场景：(1) 查询股票实时行情或历史价格，(2) 获取基金净值和业绩数据，(3) 查询指数成分股和估值，(4) 使用自然语言选股（问财），(5) 获取财务数据和估值指标。支持本地 SDK 和 HTTP API 两种调用方式。
---

...
for year in sorted(df['year'].unique()):
    ...
    if len(year_data) > 1:
        ...
        print(f"{year}: 策略 {yr_ret*100:+.2f}% | 基准 {bench_ret*100:+.2f}% | 超额 {excess*100:+.2f}% | 仓位 {avg_pos*100:.0f}%")
        if yr_ret > 0:
            positive_years += 1
        if excess > 0:
            excess_positive += 1
...
print(f"年化收益率:    {annual_return*100:+.2f}%    +11.40%    {annual_return*100 - 11.4:+.2f}%")
print(f"基准收益率:    {bench_ann*100:+.2f}%     +6.50%    {bench_ann*100 - 6.5:+.2f}%")
print(f"超额收益率:    {(annual_return-bench_ann)*100:+.2f}%     +4.90%    {(annual_return-bench_ann)*100 - 4.9:+.2f}%")

# 保存更新后的数据
df.to_excel('strategy_reproduction.xlsx', sheet_name='月度数据_完整', index=False)
print("\n已保存完整数据到 Excel")

"""
Weekly backtest - Real daily data
"""
import pandas as pd
import numpy as np
...
print()
print('='*70)
print('Weekly Backtest Results (Real Daily Data)')
print('='*70)
print('Test 1: Composite Signal (50%+30%)')
print(f'  Annual Return: {ann1*100:.2f}%')
print(f'  Benchmark: {bench1*100:.2f}%')
print(f'  Excess: {excess1*100:.2f}%')
print(f'  Sharpe: {sharpe1:.2f}')
print(f'  Max Drawdown: {dd1*100:.2f}%')
print(f'  Weeks: {n_weeks}')
print(f'  Rebalance times: {(weekly["pos_change1"] > 0).sum()}')
...
print(f'  Annual Return: {ann2*100:.2f}%')
print(f'  Benchmark: {bench2*100:.2f}%')
print(f'  Excess: {excess2*100:.2f}%')
print(f'  Sharpe: {sharpe2:.2f}')
print(f'  Max Drawdown: {dd2*100:.2f}%')
print(f'  Rebalance times: {(weekly["pos_change2"] > 0).sum()}')
...
print(f'  Annual Return: {ann3*100:.2f}%')
print(f'  Benchmark: {bench3*100:.2f}%')
print(f'  Excess: {excess3*100:.2f}%')
print(f'  Sharpe: {sharpe3:.2f}')
print(f'  Max Drawdown: {dd3*100:.2f}%')
print(f'  Rebalance times: {(weekly["pos_change3"] > 0).sum()}')

print()
print('Comparison:')
print('-'*70)
header = 'Test{:>25} Annual    Benchmark   Excess     Sharpe   MaxDD'
print(header)
print('-'*70)
row1 = 'Weekly-Composite{:>14} {:>+9.2f}% {:>+9.2f}% {:>+9.2f}% {:>7.2f} {:>9.2f}%'.format('', ann1*100, bench1*100, excess1*100, sharpe1, dd1*100)
row2 = 'Weekly-RORO{:>18} {:>+9.2f}% {:>+9.2f}% {:>+9.2f}% {:>7.2f} {:>9.2f}%'.format('', ann2*100, bench2*100, excess2*100, sharpe2, dd2*100)
row3 = 'Weekly-Sentiment{:>15} {:>+9.2f}% {:>+9.2f}% {:>+9.2f}% {:>7.2f} {:>9.2f}%'.format('', ann3*100, bench3*100, excess3*100, sharpe3, dd3*100)
print(row1)
print(row2)
print(row3)
print('-'*70)
print('Research Target{:>15} +11.40%   +6.50%    +4.90%')
